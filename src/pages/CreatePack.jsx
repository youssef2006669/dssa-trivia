...
import { createQuestionPack } from '../firebase/firestore';
...
export default function CreatePack() {
  ...
  const [questions, setQuestions] = useState([
    { type: 'mcq', question: '', options: ['', '', '', ''], correctAnswer: 0, correctAnswerText: '' }
  ]);
  ...
  const handleSubmit = async (e) => {
    ...
    // Clean up data before saving: remove unused fields based on question type
    ...
    const { id, error } = await createQuestionPack(fullPackData, user.uid);
    setLoading(false);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="bg-white p-6 sm:p-8 rounded-xl shadow-card border-t-8 border-dssa-blue">
        <h1 className="text-2xl sm:text-3xl font-black text-dssa-blue mb-6">Create Question Pack</h1>
        <form onSubmit={handleSubmit} className="space-y-8">
          
          {/* Pack Details */}
          <div className="space-y-4 border-b border-gray-200 pb-6">
            <input type="text" name="title" placeholder="Pack Title (e.g., Anatomy Wager Round)" required className="w-full px-4 py-3 text-lg border-2 border-gray-200 rounded-lg focus:border-dssa-gold outline-none font-bold text-dssa-blue" onChange={handlePackChange} />
            <textarea name="description" placeholder="Short description of this trivia pack..." required className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-dssa-gold outline-none" onChange={handlePackChange} />
          ...
          <div className="flex flex-col-reverse sm:flex-row justify-between items-stretch sm:items-center gap-4 pt-6 border-t border-gray-200">
            ...
            <button type="submit" disabled={loading} className="w-full sm:w-auto px-6 py-4 bg-dssa-gold text-white rounded-xl font-black text-lg sm:text-xl hover:bg-opacity-90 disabled:opacity-50 transition-all shadow-xl hover:-translate-y-1">
